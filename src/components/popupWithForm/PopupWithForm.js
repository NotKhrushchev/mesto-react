...
const PopupWithForm = ({title, name, submitByttonText, isOpen, onClose, onSubmit, loading, children}) => {
    return (
        ...
        <div className={`popup popup_type_${name} ${isOpen ? 'popup_opened' : ''}`}>
            <div className="popup__container">
                <form className="form form_type_profile" name={`${name}-form`} noValidate onSubmit={onSubmit}>
                    ...
                    {children}
                    <button className="form__save-btn btn" type="submit" aria-label="Сохранить">{loading ? `${submitByttonText}...` : submitByttonText}</button>
                </form>
